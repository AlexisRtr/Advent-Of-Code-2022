...
## FUNCTION GET DISTANCE BETWEEN 2 POSITTIONS
  # PARAMETERS
...
## FUNCTION CHECK IF POISITION TO ADD IS UNIQUE
  # PARAMETERS
    ## POSITION, FINAL DISTANCE
  # OUTPUT
    ## NEW POSITION FOR THE TAIL
...
crossing_bridge <- function(map){
  ...
  for (i in seq(1,n_col)){
    for (d in seq(map[i,2])){
      ...
      positions <- assign_new_position_tail(index = i, d = d, map = map, 
                                            position_head = position_head,
                                            position_tail = position_tail)
      
      position_head <- positions[[1]]
      position_tail <- positions[[2]]
      distance_temp <- positions[[3]]
      list_pos_tail <- add_pos(list_pos_tail,position_tail)
    }
  }
